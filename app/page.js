...
export default function Home() {
  ...
  const getParentRollup = (parentId) => {
    ...
    const totalEffort = children.reduce((acc, curr) => acc + (parseFloat(curr.effort) || 0), 0);
          ...
          <header className="flex justify-between items-start mb-14">
            <div>
              ...
              <div className="flex items-center gap-3 text-slate-500 font-bold uppercase tracking-widest text-[10px]">
                ...
                Real-time Modeling Engine Active v2.2
              </div>
            </div>
          </header>
            ...
            <div className="flex flex-col gap-10">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
                <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="lg:col-span-5 glass-card p-10">
                  <h4 className="text-sm font-black text-slate-400 uppercase tracking-widest mb-8">Phase Allocation</h4>
                  ...
                  <div className="flex flex-col gap-8">
                    <div>
                      <div className="text-[10px] font-black mb-3 flex justify-between uppercase text-slate-500">
                        ...
                        <span className="text-indigo-400">{Math.round(deliveryModel.offshoreRatio * 100)}%</span>
                      </div>
                      <input type="range" min="0" max="1" step="0.01" className="w-full h-1.5 bg-slate-800 rounded-lg appearance-none cursor-pointer accent-indigo-500" value={deliveryModel.offshoreRatio} onChange={(e) => setDeliveryModel({...deliveryModel, offshoreRatio: parseFloat(e.target.value)})} />
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-6">
                      <div>
                        <div className="text-[10px] font-black mb-3 flex justify-between uppercase text-slate-500">
                          <span>Hrs / Day</span>
                          <span className="text-emerald-400">{deliveryModel.hoursPerDay}h</span>
                        </div>
                        <input type="range" min="4" max="12" step="0.5" className="w-full h-1.5 bg-slate-800 rounded-lg appearance-none cursor-pointer accent-emerald-500" value={deliveryModel.hoursPerDay} onChange={(e) => setDeliveryModel({...deliveryModel, hoursPerDay: parseFloat(e.target.value)})} />
                      </div>
                      <div>
                        <div className="text-[10px] font-black mb-3 flex justify-between uppercase text-slate-500">
                          <span>Days / Mo</span>
                          <span className="text-emerald-400">{deliveryModel.workingDays}d</span>
                        </div>
                        <input type="range" min="15" max="25" step="1" className="w-full h-1.5 bg-slate-800 rounded-lg appearance-none cursor-pointer accent-emerald-500" value={deliveryModel.workingDays} onChange={(e) => setDeliveryModel({...deliveryModel, workingDays: parseFloat(e.target.value)})} />
                      </div>
                    </div>
                    ...
                    <div>
                      <div className="text-[10px] font-black mb-3 flex justify-between uppercase text-slate-500">
                        ...
                        <span className="text-amber-500">{Math.round(deliveryModel.bufferPercent * 100)}%</span>
                      </div>
                      <input type="range" min="0" max="0.2" step="0.01" className="w-full h-1.5 bg-slate-800 rounded-lg appearance-none cursor-pointer accent-amber-500" value={deliveryModel.bufferPercent} onChange={(e) => setDeliveryModel({...deliveryModel, bufferPercent: parseFloat(e.target.value)})} />
                    </div>
                  </div>
                  <p className="text-[9px] font-bold text-slate-600 uppercase tracking-widest leading-loose mt-10">Parameters drive resource costs and timeline benchmarks in real-time.</p>
                </motion.div>
              </div>
            </div>
          )}
                  ...
                  <tbody className="divide-y divide-white/[0.03]">
                     {rateCard.map(role => (
                        <tr key={role.id} className="hover:bg-white/[0.01] transition-colors">
                           <td className="font-bold text-white text-sm tracking-tight px-10 py-6">{role.role}</td>
               ...
               <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass-card">
                  <div className="flex justify-between items-center p-10 border-b border-white/5">
                     <div>
                       ...
                       <p className="text-xs font-bold text-slate-500 uppercase tracking-widest leading-none">Modeling Scope: 1.0 — 8.0 Unit Clusters</p>
                     </div>
                     <button onClick={() => setShowImporter(true)} className="px-8 py-4 rounded-xl bg-white/5 hover:bg-white/10 text-white font-black text-[10px] uppercase tracking-[2px] border border-white/10 transition-all flex items-center gap-3">
                        <Upload size={16} /> Replace RFP Scope
                      </button>
                  </div>
                     ...
                     <table className="w-full">
                           ...
                           {wbsItems.map((item, idx) => (
                              <motion.tr layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0, x: -20 }} key={item.id || idx} className={!item.isLeaf ? 'bg-white/[0.02] group' : 'group hover:bg-white/[0.01] transition-colors'}>
                                 ...
                                 <td className="px-8 py-5">
                                    <input className={`bg-transparent border-none outline-none w-full ${!item.isLeaf ? 'font-black text-base text-white' : 'font-bold text-slate-400 text-sm'}`} value={item.deliverable} onChange={(e) => updateWbsItem(item.id, 'deliverable', e.target.value)} />
                                 </td>
                                 ...
                                 <td className="px-8 py-5 font-black tabular-nums text-sm">
                                    <span className={item.isLeaf ? "text-emerald-400" : "text-white/40 italic"}>${Math.round(item.isLeaf ? (item.effort * metrics.blendedRate) : getParentRollup(item.id).totalCost).toLocaleString()}</span>
                                 </td>
                        ...
                        <tfoot className="bg-slate-900/50 border-t-2 border-indigo-600/50">
                           <tr className="font-black">
                              <td colSpan="3" className="text-right py-8 px-10 text-[10px] uppercase text-slate-400">WBS deliverable total:</td>
